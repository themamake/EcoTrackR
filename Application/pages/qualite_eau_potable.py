...
def get_Hydrometric_stations_in_bbox(bbox):
    ...
    """
    url = "API_ENDPOINT_QUALITE_EAU_1?"
...
def get_api_debit(code_station):
    ...
    """
    url = "API_ENDPOINT_QUALITE_EAU_2"
...
def get_api_history_debit(code_station):
    ...
    url = "API_ENDPOINT_QUALITE_EAU_3"
...
if __name__ == "__main__":
    main()
# Check the user's selected endpoint from the session state
endpoint = st.session_state.get('selected_endpoint', 'Paramètres physico-chimiques')

base_url = "https://hubeau.eaufrance.fr/api/v1/qualite_eau_potable/"

# Mapping endpoint selection to API endpoint
endpoint_mapping = {
    "Paramètres physico-chimiques": "parametres_physicochimiques",
    "Pesticides": "pesticides",
    "Microbiologie": "microbiologie",
    "Radioactivité": "radioactivite"
}

# Fetching data from the selected API endpoint
response = requests.get(base_url + endpoint_mapping[endpoint], params={"size": 1000})  # Fetching 1000 records for demonstration
data = response.json()
...
if 'data' in data:
    ...
    # Displaying histogram or bar chart based on the available data columns
    if 'resultat' in df.columns and 'nom_parametre' in df.columns:
        st.write(f"Visualisation pour {endpoint}")
        for param in df['nom_parametre'].unique():
            st.subheader(param)
            df[df['nom_parametre'] == param]['resultat'].plot(kind='hist')
            st.pyplot(plt)
            plt.clf()
else:
    st.write("Erreur lors de la récupération des données. Veuillez essayer à nouveau.")
